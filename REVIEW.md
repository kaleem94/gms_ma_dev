...
model.Song ──▶ bars.BarMap ──▶ segment.segment_song ──▶ PatternContent ──▶ export.pattern_to_bytes ─┐
   ...
   │                                                                                              ▼
   │                                       features / music_theory / harmony / tag        store.Repository
   │                                                    │  (metrics, key, chords)          db/midi_loops.db
   │                                                    ▼                                  (patterns, placements,
   └─ indexer.run_index ──────────────────────────▶ tags + features                         motif_hits, assets,
                                                                                            stems, manifests)
                                                                                                  │
                                                                        indexer.build_manifest ◀──┘
                                                                                  │
                                          viewer._Handler (HTTP) ──▶ /api/* + assets/js/{timeline,library}
```
...
**Why**: Mirrors the motif miner's event model so chord analysis works on the same representation; accepts both note tuples and `model.Note`.
...
**Why**: Mirrors the viewer's event building but sources everything from the repository (song PPQ/initial tempo, track channel/program, stem file/BLOB), so library audition and DAW export agree on timing.
...
### song_play_events / pattern_play_events / _offset_events <!-- ref:gms_ma/viewer.py:204-298 -->
**Purpose**: Build absolute-time event lists for a whole song (with optional mute/solo track set) and for one pattern/stem; `_offset_events` re-schedules a seek.
**Why**: Reconstruction from the manifest means playback uses exactly the analysed placements. `_offset_events` drops already-sounded notes but re-emits program changes at `t0` so a mid-loop seek keeps the instrument patch.
**Data Flow**: `manifest (+ enabled tracks / pattern id / render) → [(sec, kind, ch, a, b)]`.
**Relationships**: Used by `_do_POST` play endpoints.

### _Handler <!-- ref:gms_ma/viewer.py:305-661 -->
**Purpose**: The `BaseHTTPRequestHandler` implementation: static assets, JSON APIs (`/api/songs`, `/api/song`, `/api/library`, `/api/library/count`, `/api/library/download`, `/api/midi-ports`, `/api/status`, `/api/about`, `/license`) and POST actions (`/api/play`, `/api/play-pattern`, `/api/library/play`, clipboard/copy-ch1/sanitize, `/api/stop`).
**Why**: stdlib only, no framework. All DB access is serialised through `self.repo.lock` because the SQLite connection is shared across request threads. `_write_body` gzips compressible bodies and swallows client-disconnect errors. Static serving is traversal-guarded.
**Data Flow**: HTTP request → JSON/MIDI/HTML; POST bodies drive the shared `_Player`.
**Relationships**: Serves the `assets/` UI; uses `build_manifest`, `library`, `clipboard`, `midi_out`.

### serve / _bind_server / _QuietThreadingHTTPServer <!-- ref:gms_ma/viewer.py:718-786 -->
**Purpose**: Start the threaded server, warm the library catalog in a background thread (on a separate connection), and optionally open a browser.
**Why**: `_QuietThreadingHTTPServer` suppresses tracebacks for routine aborts; the warm-up avoids the first `/api/library` request paying the catalog build; a separate connection keeps the shared lock free.
**Data Flow**: `(repo, port, open_browser) → running server`.
**Relationships**: Entry point for `cli.cmd_view` and `gui._open_viewer` (via `python -m gms_ma view`).

## gms_ma/cli.py
### cmd_index (plus _build_parser / main) <!-- ref:gms_ma/cli.py:14-53 -->
**Purpose**: Parse arguments and dispatch every subcommand (`index`, `list`, `manifest`, `reconstruct`, `export`, `view`, `gui`, `tag`, `motifs`, `delete`, `doctor`).
**Why**: A flat argparse table keeps the CLI discoverable; `index` prints per-file timing on stderr and a JSON summary on stdout (including `elapsed`) so it is scriptable. `--harmony-split off|top|onset` controls separation.
**Data Flow**: `argv → AnalysisConfig → run_index → JSON`.
**Relationships**: Thin wrapper over `indexer`, `library`, `viewer`, `gui`, `doctor`.

## gms_ma/gui.py
### IndexerGUI._start / _worker / _poll / _finish <!-- ref:gms_ma/gui.py:216-393 -->
**Purpose**: tkinter indexer that picks a folder or files, shows progress, and drives `run_index` on a worker thread with a queue back to the UI.
**Why**: The UI thread must stay responsive and tkinter is not thread-safe, so all updates flow through a `queue` polled by `_poll`. Already-indexed files are skipped via `classify_indexed_ex`, with “Re-index already processed” and “Check DB / Fix corrupt” escape hatches.
**Data Flow**: `files, db, out, cfg → queue messages (progress/file/log/done/error)`.
**Relationships**: Uses `indexer`, `doctor`, and launches `view` as a detached process.

## gms_ma/doctor.py
### scan / fixable <!-- ref:gms_ma/doctor.py:37-94 -->
**Purpose**: Detect corrupted songs (no tracks, dangling placements, patterns/stems without assets, invalid manifest JSON, missing source file) with an optional deep check.
**Why**: Failed analyses are rolled back, but older DBs can hold partial songs; `scan(deep=)` separates a cheap structural check (used by the GUI on re-index) from a thorough one.
**Data Flow**: `repo → [finding{name,path,issues}]`; `fixable()` keeps those whose source file still exists.
**Relationships**: Used by `cli.cmd_doctor`, `indexer.corrupt_songs`, and the GUI Check DB / Fix corrupt buttons.

## gms_ma/clipboard.py / gms_ma/midi_out.py
### copy_file_to_clipboard / WinMidiOut <!-- ref:gms_ma/clipboard.py:78-120 -->
**Purpose**: Windows-only helpers via `ctypes`: put a file on the clipboard as `CF_HDROP` (so it can be pasted into Explorer/DAW) and send raw MIDI messages through winmm.
**Why**: No third-party dependency; both degrade gracefully (`supported()` / `available()`) so the app still works visually and on non-Windows.
**Data Flow**: file path → clipboard; `(channel, note, velocity)` → winmm short messages.
**Relationships**: Used by `viewer` endpoints and `_Player`.

## assets/js/common/components/miniPlayer.js
### mountMiniPlayer <!-- ref:assets/js/common/components/miniPlayer.js:19-183 -->
**Purpose**: The shared bottom transport used by both pages: play/pause, info, current/total time, seek, a 0.25–3× speed slider, and stop. It owns the local position clock and the fetch calls.
**Why**: One component means the timeline (whole song + motif auditions) and the library loop player behave identically. Pattern playback uses `/api/library/play` (seek via `t`, hold for `end_s`, returns `total_s`); full songs use `/api/play`. The clock advances `pos += (now-last)*speed`; changing speed mid-playback re-streams from the current position so the server stays in sync. Session-only speed avoids surprising persisted state.
**Data Flow**: host calls `playPattern`/`playSong` → `stream(tSec)` POST → `st` mutated → `onTick`/`onSpeed` callbacks.
**Relationships**: Mounted by `timeline/app.js` and `library/app.js`; `exportActions.js` is the sibling for downloads.

## assets/js/common/exportActions.js
### downloadUrl / downloadMidi / copyMidi / copyDawCh1 / dawExport <!-- ref:assets/js/common/exportActions.js:5-62 -->
**Purpose**: Shared pattern download/clipboard actions for both pages, forwarding an optional export `bpm`.
**Why**: De-duplicates the library's former inline helpers and gives the timeline the same behaviour; `bpm` is passed through so the served/copied MIDI is retempoed to the current speed.
**Data Flow**: `{pattern_id, render, bpm, mode} → download URL / POST body`.
**Relationships**: Used by `timeline/app.js` (`doExport`) and `library/app.js` (`expDownload`/`expCopy`/`expCopyCh1`/`expDaw`); endpoints in `viewer._do_POST` and `_library_download`.
...
## assets/js/common/settings.js / settingsPanel.js / theme.js / api.js
### SettingsStore / mountSettingsPanel / ThemeController / ApiClient <!-- ref:assets/js/common/settings.js:20-80 -->
**Purpose**: A localStorage-backed observable settings store (`theme`, opacities, `motifLayout`, `showHarmony`), a gear popover that edits it, a theme controller that resolves auto/light/dark/night and exposes the canvas palette, and a thin JSON API client.
**Why**: Preferences are shared between the two pages via one key (with a legacy-key migration). The theme controller reads CSS custom properties so canvas drawing matches the DOM. `ApiClient` centralises fetch so views depend on a small interface, not URL strings.
**Data Flow**: `set() → save() → _emit()`; `SettingsStore` → `settingsPanel` (sync/onChange) and `timeline/app.js` via `SET`.
**Relationships**: Used by both page apps and `aboutPanel`.

## assets/js/timeline/layout.js
### packLanes / largestPerRegion / layoutMotifs <!-- ref:assets/js/timeline/layout.js:14-51 -->
**Purpose**: Pure layout math turning overlapping motif hits into clean lanes.
**Why**: `lanes` stacks overlapping hits into sub-rows; `largest` keeps only the biggest motif per region. No DOM/state, so it is unit-tested via Node.
**Data Flow**: `hits, mode → [[hit…]…]`.
**Relationships**: Used by `timeline/app.js` `layoutMotifRows`/`layoutHarmonyRows`.

### groupByLayer / summarizeLane <!-- ref:assets/js/timeline/layout.js:57-88 -->
**Purpose**: Group blocks into `{mixed, melody, harmony}` and summarise a lane (hit/family counts + dominant layer/chord/harmony).
**Why**: Pure helpers so the Harmony view and per-lane labels stay testable.
**Data Flow**: `blocks → groups` / `blocks → {hits, families, layer, chord, harmony}`.
**Relationships**: Used by `timeline/app.js` `layoutHarmonyRows`, `drawLaneLabel`.
...
### timeStep / timeTicks <!-- ref:assets/js/timeline/layout.js:96-108 -->
**Purpose**: Choose a “nice” seconds step for the adaptive ruler so labels are never closer than `minPx`.
**Why**: Keeps the time axis readable at every zoom level instead of crowding/stretching.
**Data Flow**: `secPerPx, minPx → step`; `totalSec, … → [0, step, 2*step…]`.
**Relationships**: Used by `timeline/app.js` `timeLabels`.

## assets/js/timeline/app.js
### Overview <!-- ref:assets/js/timeline/app.js:1-58 -->
**Purpose**: The timeline composition root: state (`state`, `selection`, `hover`, `t0`, `viewMode`), the shared mini player, settings/theme, and the canvas.
**Why**: A single module keeps the many view renderers sharing one state object and one palette; the mini player is created once and drives both whole-song and motif playback.
**Data Flow**: `/api/song` manifest → `state` → renderers → canvas.
**Relationships**: Imports `layout.js`, `miniPlayer`, `exportActions`, `settingsPanel`, `aboutPanel`, `gm`, `theme`, `api`.

### tickX / xTick / timePts / secAtTick / tickAtSec <!-- ref:assets/js/timeline/app.js:117-159 -->
**Purpose**: Map ticks↔pixels (bar-linear) and ticks↔seconds (tempo-map aware) for drawing and seeking.
**Why**: The x-axis is bar-based (linear in bars) but time readouts/playhead are in seconds, so two conversions coexist; `timePts` builds the tick/second control points from `state.bars`.
**Data Flow**: `tick ↔ x`, `tick ↔ sec`.
**Relationships**: Used by every renderer, the ruler, seeking and the playhead.

### motifBlocksOf / layoutMotifRows / layoutHarmonyRows <!-- ref:assets/js/timeline/app.js:171-260 -->
**Purpose**: Turn motif families + hits into drawable blocks and cache two layouts: the Motifs view (all hits packed per track) and the Harmony view (per-track sections for mixed/melody/harmony).
**Why**: Blocks carry `layer`/`chord`/`harmony` from the family tags so the Harmony view and lane labels can annotate them. Layouts are cached keyed on `state` identity + `SET.motifLayout` so per-frame playback highlighting stays cheap.
**Data Flow**: `state.tracks[].motifs → rows{sections, lanes, summaries}`.
**Relationships**: `layout.js` (`layoutMotifs`, `groupByLayer`, `summarizeLane`); consumed by `drawMotifLane`/`renderMotifs`/`renderHarmony`.

### activeMotifHits / motifHitAt / laneAtSection <!-- ref:assets/js/timeline/app.js:262-302 -->
**Purpose**: Highlight motif hits under the global playhead during whole-song playback, and hit-test the motif/harmony lane stacks for click selection.
**Why**: Reading from the active view's cached rows keeps both views consistent; `laneAtSection` handles the Harmony view's nested sections.
**Data Flow**: `tick → Set(hit)`; `(y, tick) → {block, track}|null`.
**Relationships**: Used by `renderMotifs`/`renderHarmony` and `hitAtPos`.

### describeBlock / tagValue <!-- ref:assets/js/timeline/app.js:313-337 -->
**Purpose**: One-line hover description of a block, including layer/chord/progression when `showHarmony` is on.
**Why**: `tagValue` reads `tags[kind][0].value` (tag_groups returns lists), fixing an earlier bug where the raw array was stringified.
**Data Flow**: `(track, block) → string`.
**Relationships**: Used by the hover bar and tip.

### showPatternInfo / renderExport / doExport <!-- ref:assets/js/timeline/app.js:414-507 -->
**Purpose**: Build the sidebar for the selected/hovered pattern: track, size, confidence, audition buttons, tag chips, an optional Harmony block, and an Export section (render picker, export BPM, download/copy actions).
**Why**: The export BPM defaults to `song BPM × speed` and follows the speed slider until the user edits it (tracked by `exportBpmEdited`, reset on selection change). Clipboard buttons disable when `/api/midi-ports` reports no clipboard backend.
**Data Flow**: `selection|hover → HTML`; `doExport(kind) → downloadMidi/copyMidi/copyDawCh1/dawExport`.
**Relationships**: Uses `exportActions.js`, `mini.state().speed`, `store`-served tags/stems.

### renderArrangement / drawMotifLane / renderMotifs / renderHarmony / renderRepetition <!-- ref:assets/js/timeline/app.js:591-949 -->
**Purpose**: The four canvas renderers. Arrangement draws one block per placement per track; Motifs/Harmony draw motif lanes (shared `drawMotifLane`) with per-lane labels; Repetition draws the per-level compression view.
**Why**: `drawMotifLane` centralises block styling (selection, audition progress, pulsing “playing” halo, on-canvas chord labels) so the two motif views stay identical. The Harmony view adds coloured per-layer section headers.
**Data Flow**: `state + layout rows → canvas pixels`.
**Relationships**: `layout.js`, `activeMotifHits`, the ruler/grid helpers, the mini player progress.

### zoomAt / zoomFit / hitAtPos / applySelection / startPlay / playBlock <!-- ref:assets/js/timeline/app.js:994-1237 -->
**Purpose**: Zoom (anchored under the cursor), fit, pointer hit-testing, selection/audition, whole-song playback via the mini player, and isolated motif/pattern audition with progress highlight.
**Why**: Whole-song playback runs through the shared transport so its seek bar scrubs the song; `t0` mirrors `mini.state().pos`. Motif clicks play only that pattern (via `/api/library/play`) and highlight the single block, while ruler drags seek and re-stream on release. `startPlay` re-streams with the current mute/solo mix.
**Data Flow**: pointer/zoom events → `t0`/`state` → mini player requests → renders.
**Relationships**: `miniPlayer`, `exportActions`, `library` endpoints, `layout` rows.

## assets/js/library/app.js
### fetchLib / beginLoading / renderAll <!-- ref:assets/js/library/app.js:84-593 -->
**Purpose**: The library composition root: JIT-paged catalogue fetch, filter/search/song state, flat/tree rendering, and a loading indicator.
**Why**: The server pages the catalogue so large libraries open fast; the loader shows a real total from the cheap `/api/library/count` and animates a “processed / total” counter (stepping by 1000) while the main request is in flight. Tree view fetches song summaries first and loops lazily.
**Data Flow**: filters → `/api/library` → `data` → list/detail/pager.
**Relationships**: Uses `library` API endpoints; `renderDetail` links to the timeline and hosts export.

### renderDetail / exportOpts / expDownload / expCopy / expCopyCh1 / expDaw <!-- ref:assets/js/library/app.js:504-615 -->
**Purpose**: The selection detail panel (source, instrument, loop stats, tags, export) plus export wrappers.
**Why**: `exportOpts` bundles the selected pattern, current render and export BPM (`song BPM × speed`, editable) and passes them to the shared `exportActions`; the detail panel shows the export BPM field and clipboard-aware buttons.
**Data Flow**: `selected → HTML`; export actions → shared module.
**Relationships**: `exportActions.js`, `mini.state().speed`, `settingsPanel`/`aboutPanel`.

## tests
### Python + JS test suites <!-- ref:tests -->
**Purpose**: `tests/` covers parsing, segmentation, motif mining, harmony/chord detection, features/tags, store/delete/doctor, CLI commands, the viewer APIs (including `/api/about`, `/api/library/count`, export BPM), static assets, GUI wiring, and JS modules.
**Why**: The pure modules (`features`, `music_theory`, `harmony`, `bars`, `layout.js`, `pager.js`, `forest.js`) are unit-tested directly; the Node-harness tests (`test_js_layout`, `test_js_forest`, `test_js_pager`, `test_js_gm`, `test_js_syntax`) parse/exercise the browser modules without a DOM. `test_js_syntax` globs every `assets/js/**/*.js` and runs `node --check`.
**Data Flow**: `pytest` → subprocess `node --check`/inline ESM for JS; HTTP tests spin up `viewer._Handler` on an ephemeral port.
**Relationships**: `tests/conftest.py` provides `write_midi`, `repetitive_song`, `chord_bars`, `index_one` used across suites.

## Cross-cutting observations & risks
### Suggested follow-ups <!-- ref: overall -->
- **`assets/js/timeline/app.js` is very large** (~1,240 lines) and mixes state, time math, four renderers, interaction and export. Consider extracting the renderers and the time/zoom model into modules (mirroring the existing `layout.js` split) to keep each testable and to reduce merge risk.
- **Export state is duplicated** across the two page apps (`exportBpm`/`exportBpmEdited`, render pickers, BPM sync). A small shared `exportPanel`/state helper (like `miniPlayer`) would remove the parallel logic and the `onSpeed` wiring.
- **`miniPlayer` couples transport to export**: `onSpeed` is used only to refresh an export field. It works, but a pub/sub on `state()` (or the `SettingsStore` observable) would decouple concerns.
- **`detect_chord` ties are order-dependent**: e.g. `{0,4,7,9}` resolves to `C6` rather than `Am7` because root 0 is tried first. It is deterministic and defensible, but worth documenting or biasing by bass/function.
- **`retempo_midi` assumes a tempo meta exists or can be inserted at `tracks[0]`**: fine for the current exporter, but files with no tracks would be silently returned unchanged (the `and mf.tracks` guard). Low risk.
- **Threaded server + shared SQLite**: all repository access goes through `Repository.lock`, and `_warm_catalog` uses a separate connection; keep this invariant when adding endpoints, since a missed lock would surface as intermittent `sqlite3` errors.
- **Windows-only audio/clipboard**: `midi_out`/`clipboard` degrade gracefully, but they cannot be exercised in CI; the playback-speed change therefore has no automated timing test (only the clamp is unit-tested).
